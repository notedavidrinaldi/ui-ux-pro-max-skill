...
# Strict RTL filtering for a domain search
python3 .claude/skills/ui-ux-pro-max/scripts/search.py "Arabic dashboard" --domain style --rtl=full
...
The `--rtl` mode uses the `rtl_level` field for product/style/color entries.
Use bare `--rtl` to include all compatible entries, or select one level:

- `--rtl=full` – native RTL-friendly
- `--rtl=partial` – needs minor RTL adjustments
- `--rtl=caveats` – usable with additional checks
