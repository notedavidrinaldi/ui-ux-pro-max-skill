...
Usage: python search.py "<query>" [--domain <domain>] [--stack <stack>] [--max-results 3]
       ...
       python search.py "<query>" --domain style --rtl[=full|partial|caveats]
...
RTL filtering (style, color, and product domains):
  --rtl        Include every RTL-compatible level
  --rtl=LEVEL  Include only full, partial, or caveats
...
import sys
import io
from core import CSV_CONFIG, AVAILABLE_STACKS, MAX_RESULTS, RTL_LEVELS, UNTRUNCATED_COLS, search, search_stack
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="?",
        const="all",
        choices=["all", *RTL_LEVELS],
        default=None,
        help="Filter by RTL compatibility level in style/color/product searches",
    )
    # Persistence (Master + Overrides pattern)
    ...
    # Domain search
        ...
        result = search(args.query, args.domain, args.max_results, rtl=args.rtl)
        if args.json:
            print(json_module.dumps(result, indent=2, ensure_ascii=False))
        else:
            print(format_output(result, full=args.full))
